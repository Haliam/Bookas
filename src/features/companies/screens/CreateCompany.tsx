...
export function CreateCompany() {
  ...
  if (step === "done") {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
        <div className="w-24 h-24 rounded-full bg-[#E8FBF4] flex items-center justify-center text-4xl mb-6">🏢</div>
        ...
        <p className="text-sm text-[#9CA3AF] text-center mb-8 max-w-[260px]">
          Tu negocio está listo. Ahora añade tus servicios para empezar a recibir reservas.
        </p>
        <div className="w-full flex flex-col gap-3">
          <Button fullWidth size="lg" onClick={() => navigate("/provider/companies/c1/services")}>
            Añadir servicios
          </Button>
          <Button variant="ghost" fullWidth onClick={() => navigate("/provider/companies")}>
            Ver mis negocios
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      <TopBar title={step === "info" ? "Nuevo negocio" : "Horarios"} back light />
      ...
      <div className="px-5 py-5 flex flex-col gap-5">
          ...
          <>
            ...
            <div className="bg-white rounded-2xl p-6 flex flex-col items-center justify-center gap-3 border-2 border-dashed border-[#E0E0E0] min-h-[140px]">
              ...
              <div className="text-center">
                <p className="text-sm font-medium text-[#374151]">Foto del negocio</p>
                <p className="text-xs text-[#9CA3AF]">Toca para subir imagen · 5MB máx</p>
              </div>
            </div>
        ...
        {step === "hours" && (
          <>
            <p className="text-sm text-[#9CA3AF]">Configura tus horarios de atención</p>
            <div className="bg-white rounded-2xl shadow-[0_1px_6px_rgba(0,0,0,0.05)] overflow-hidden">
              {["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"].map((day, i) => {
                const closed = i === 6;
                return (
                  <div key={day} className={`flex items-center gap-3 p-4 ${i > 0 ? "border-t border-[#F4FAF4]" : ""}`}>
                    <div className="w-20">
                    ...
                    {closed ? (
                      ...
                      <div className="flex items-center gap-2 flex-1">
                        <input type="time" defaultValue="09:00" className="flex-1 h-9 border border-[#F0F0F0] rounded-xl px-3 text-sm text-[#2C2C2C] focus:outline-none focus:border-[#E0E0E0]" />
                        <span className="text-[#9CA3AF]">—</span>
                        <input type="time" defaultValue="20:00" className="flex-1 h-9 border border-[#F0F0F0] rounded-xl px-3 text-sm text-[#2C2C2C] focus:outline-none focus:border-[#E0E0E0]" />
                      </div>
                    )}
                  </div>
                );
