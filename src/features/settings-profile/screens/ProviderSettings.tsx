...
export function ProviderSettings() {
  ...
  const Switch = ({
    ...
    >
      <div
        className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-all duration-200 ${value ? 'left-6' : 'left-0.5'}`}
      />
    </button>
  )
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="px-5 py-5 flex flex-col gap-5">
        {/* Business profile */}
        <div>
          <p className="text-xs text-[#9CA3AF] uppercase tracking-widest mb-2 px-1">
            Perfil profesional
          </p>
          <div className="bg-white rounded-2xl p-5 shadow-[0_1px_6px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-4 mb-4 pb-4 border-b border-[#F4FAF4]">
              ...
              <div>
                ...
                <span className="text-[10px] bg-[#FEF7E0] text-[#D4950A] px-2 py-0.5 rounded-full">
            ...
            {[
              ...
              <div
                key={label}
                className="flex items-center justify-between py-3 border-b border-[#F4FAF4] last:border-0"
              >
                <div>
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl shadow-[0_1px_6px_rgba(0,0,0,0.05)] overflow-hidden">
            <div className="flex items-center justify-between p-4">
              ...
              <Switch
                value={bookingSettings.autoConfirm}
                onChange={() => toggleBooking('autoConfirm')}
              />
            </div>
            <div className="flex items-center justify-between p-4 border-t border-[#F4FAF4]">
              <div className="flex-1 pr-4">
                ...
                <p className="text-xs text-[#9CA3AF] mt-0.5">
                  15 min de descanso entre servicios
                </p>
              </div>
              <Switch
                value={bookingSettings.buffer}
                onChange={() => toggleBooking('buffer')}
              />
            </div>
          </div>
        </div>
            ...
            {[
              {
                ...
                label: 'Nueva reserva',
                desc: 'Alerta cuando recibo una cita',
              },
              {
                key: 'cancellation' as const,
                label: 'Cancelación',
                desc: 'Cuando un cliente cancela',
              },
              {
                key: 'reminder' as const,
                label: 'Recordatorio',
                desc: '1h antes de cada cita',
              },
              {
                key: 'review' as const,
                label: 'Nueva reseña',
                desc: 'Cuando te valoran',
              },
              {
                key: 'marketing' as const,
                label: 'Novedades de BookAs',
                desc: 'Tips y actualizaciones',
              },
            ].map(({ key, label, desc }, i) => (
              <div
                key={key}
                className={`flex items-center justify-between p-4 ${i > 0 ? 'border-t border-[#F4FAF4]' : ''}`}
              >
                <div>
        ...
        <div className="bg-[#2C2C2C] rounded-2xl p-5">
          ...
          <p className="text-white/70 text-xs mb-4">
            32/50 reservas este mes · 1 negocio activo
          </p>
          <Button
            variant="secondary"
            size="sm"
