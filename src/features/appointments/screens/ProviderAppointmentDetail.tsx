...
export function ProviderAppointmentDetail() {
  ...
  const appt = PROVIDER_APPOINTMENTS.find(a => a.id === id) || PROVIDER_APPOINTMENTS[0];
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-5 py-5 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-[0_1px_8px_rgba(0,0,0,0.05)]">
          ...
          {[
            { icon: Calendar, label: "Fecha", value: new Date(appt.date + "T12:00").toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long" }) },
            { icon: Clock, label: "Hora", value: `${appt.time} · ${appt.duration} minutos` },
            { icon: User, label: "Profesional", value: appt.providerName },
          ].map(({ icon: Icon, label, value }) => (
            <div key={label} className="flex items-center gap-3 py-2.5 border-t border-[#F4FAF4]">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-[0_1px_6px_rgba(0,0,0,0.05)]">
          <p className="text-xs text-[#9CA3AF] uppercase tracking-widest mb-3">Cliente</p>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#2C2C2C] flex items-center justify-center">
              <span className="text-white font-medium text-sm">S</span>
            </div>
            <div>
              <p className="text-sm font-medium text-[#111827]">Sofía Ramírez</p>
              <p className="text-xs text-[#9CA3AF]">sofia@email.com · +34 612 345 678</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#FAFAFA] rounded-2xl p-4 border border-[#F0F0F0]">
          ...
          <p className="text-sm text-[#374151]">Sin notas adicionales para esta cita.</p>
        </div>
